...
class UserInfo extends Component {
  ...
  componentDidMount = () => {
    ...
    this.props.getAllServiceListRequest({ technicianId: this.props.match.params.id })
    ...
    this.props.getTechinicianTimeLogRequest({ technicianId: this.props.match.params.id })
    this.setState({
      userId: userId,
      activeTab: query.tab
  ...
  render() {
    ...
    return (
      <div className={"p-3"}>
        <div className={"pb-3"}>
          <h3>{technicianData ? `${technicianData.firstName} ${" "} ${technicianData.lastName}` : null}</h3>
          {
            <div>
              <i className={"fa fa-phone"} /> {technicianData.phone || "Not Added"}
            </div>
          }
          {" "}
          <div><i className={"fa fa-envelope"} /> {technicianData && technicianData.email ? technicianData.email : <span className={"text-muted"}>Email is not updated</span>}
          </div>
        </div>
