...
class TechnicianTimer extends Component {
  ...
  componentDidUpdate = ({ userReducer }) => {
    ...
    if (userNewReducer && userNewReducer.users && userNewReducer.users.length && (userReducer.users !== userNewReducer.users)) {
        ...
        <Row>
          {
            userData && userData.length ?
              userData.map((users, index) => {
                ...
                const isWorking =
                  users && users.currentlyWorking && (users.currentlyWorking.orderId || users.currentlyWorking.generalService || timer)
                    ? true
                    : false;

                const startTime =
                  users && users.currentlyWorking &&
                    users.currentlyWorking.startTime
                    ? users.currentlyWorking.startTime
                    : false;
                const technicicanService = serviceData.filter(service => service.technician === users._id)
                technicicanService.map((data) => {
                  if (users._id === data.technician) {
                    const dataObject = {
                      label: `Order(#${data.orderId.orderId}) ${data.orderId.orderName}`,
                  ...
                  <Col key={index} md={"3"}>
                    <Card className={'text-center timer-display'}>
                      <div className={isWorking ? "isWorking" : ""}>
                        <CardBody>
                          <div className={"pb-2"}>
                            <Avtar value={users.firstName} class={"avtar-component"} />
                          </div>
                          ...
                          <FormGroup className={"d-flex align-items-center"}>
                            ...
                            <Select
                              ...
                              value={(users.selectedOrder.value !== '') ? users.selectedOrder : defaultOrderSelect}
                              onChange={(e) => this.handleOrderSelect(e, index)}
                              type="select" />
                          </FormGroup>
                          {!isWorking ? (
                            <Button onClick={users.selectedOrder.value !== "" ? () => this.startTimer(users._id, users.selectedOrder.value) : () => this.startTimer(users._id)}>Clock In</Button>) :
                            (
                              <Button
                                color={"danger"}
                                onClick={() => this.stopTimer(users._id, users.currentlyWorking.orderId, users.currentlyWorking.serviceId)}
                              >
                                Clock Out
                          </Button>
                            )}
